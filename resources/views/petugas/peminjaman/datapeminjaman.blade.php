            ...
            <table class="table table-striped">
                ...
                <tbody>
                    @php
                        $no=1;
                    @endphp
                    @foreach ($peminjaman as $no => $datapeminjaman)
                    <tr>
                        <td>{{ $no+$peminjaman->firstItem() }}</td>
                        <td>{{ $datapeminjaman->user ? $datapeminjaman->user->name : 'Nama tidak ditemukan' }}</td>
                        <td>{{ $datapeminjaman->buku ? $datapeminjaman->buku->judul : 'Buku tidak ditemukan' }}</td>
                        <td>
                            <img src="{{ asset('fotobuku/' . $datapeminjaman->buku->foto) }}" alt="foto"
                                style="width : 30px">
                        </td>
                        <td>{{ $datapeminjaman->tanggal_peminjaman }}</td>
                        <td>{{ $datapeminjaman->tanggal_pengembalian }}</td>
                        <td>{{ $datapeminjaman->status_peminjaman}}</td>
                        {{-- <td>
                          <a href="/tampilkandata/{{ $datapeminjaman->id }}" class="btn btn-info btn-sm"><i class="bi bi-pencil-square"></i> Edit</a>
                          <button class="btn btn-danger btn-sm delete" data-id="{{ $datapeminjaman->id }}" data-nama="{{ $datapeminjaman->nama_buku }}"><i class="bi bi-trash"></i> Delete</button>
                        </td> --}}
                      </tr>
                    @endforeach
                </tbody>
